---
name: Bug Report
about: Report a bug to help us improve
title: '[BUG] '
labels: bug, triage
assignees: ''
---

## Bug Description

<!-- Clear description of what went wrong -->

## Component Affected

- [ ] profile
- [ ] assess
- [ ] export / export-star
- [ ] compare
- [ ] CLI (argument parsing, error messages)
- [ ] Documentation
- [ ] Other: _______________

## Command Used

<!-- Exact command you ran — use the full interpreter path if PATH is unreliable -->

```
C:\Users\<you>\micromamba\envs\dqt\python.exe -m data_quality_toolkit.cli.main <subcommand> <args>
```

## CLI Output / stderr

<!-- Paste the exact terminal output, including any error messages -->

```
paste output here
```

## Dataset / Demo Path

<!-- e.g. examples/demo/Uber_Data.csv, or describe the CSV structure -->

## Expected Behavior

<!-- What you expected to happen -->

## Actual Behavior

<!-- What actually happened -->

## Environment

- **OS**: [e.g., Windows 11]
- **Python Version**: [e.g., 3.12.x]
- **DQT Version / commit**: [e.g., v1.0.0-internal or commit hash]
- **Installation method**: [source / pip]

## Additional Context

<!-- Anything else that might help diagnose the issue -->
