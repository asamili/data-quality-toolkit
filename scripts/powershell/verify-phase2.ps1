...
if (-not $built) {
  ...
  if (Get-Command dqt -ErrorAction SilentlyContinue) {
    ...
    Run-Checked $PY ($PY_ARGS + @("-m","data_quality_toolkit.adapters.cli.main","export","test.csv"))
    Run-Checked $PY ($PY_ARGS + @("-m","data_quality_toolkit.adapters.cli.main","build-pbi","--star","dist/star","--out","dist/powerbi_package"))
  }
}
