...
`sample_orders.csv` is fully synthetic with no real names, addresses, or identifiers.
...
`sample_orders.csv` is a synthetic 170-row business orders dataset designed to trigger realistic quality issues so you can see DQT's issue detection in action.
...
```bash
dqt export examples/demo/sample_orders.csv --outdir dist/demo   # run twice
