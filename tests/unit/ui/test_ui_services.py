...
def test_collect_settings_snapshot_redacts() -> None:
    snapshot = _collect_settings_snapshot()
    assert snapshot["api_key"] == "REDACTED"
    assert snapshot["secret_token"] == "REDACTED"  # noqa: S105


def test_probe_writable_dir_success(tmp_path: Path) -> None:
    success, err = _probe_writable_dir(str(tmp_path))
    assert success is True
    assert err is None


def test_probe_writable_dir_missing() -> None:
    success, err = _probe_writable_dir("nonexistent_dir")
